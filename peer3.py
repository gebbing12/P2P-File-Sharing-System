...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 12 21:48:13 2021
...
with open('FILE_LIST_peer3.json','r') as f:
    FILE_LIST= json.load(f)

with open('REGISTER_LIST_peer3.json','r') as f:
...
def download(chunkdict):
        ...
        with open('FILE_LIST_peer3.json','w') as f:
            json.dump(FILE_LIST,f)
        with open('REGISTER_LIST_peer3.json','w') as f:
...
def login(SERVER_ADDR, SERVER_PORT):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        try:
            ...
            data = sock.recv(1024)   #make sure the server update successfully
            print(int(data))
            sock.close()
        except:
...
if __name__ == '__main__':
        inputs = []
        login(SERVER_ADDR, SERVER_PORT)
        print('khbhbojbo')
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as peer_socket:
            peer_socket.setblocking(False)
            peer_socket.bind(('127.0.0.1', 7773))
